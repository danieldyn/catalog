...
modest_rancho  oci://unikraft.org/base:latest  /usr/bin/node /usr/src/server.js  7 seconds ago  running  1024M  0.0.0.0:3000->3000/tcp  qemu/x86_64
```

The instance name is `modest_rancho`.
To close the Unikraft instance, close the `kraft` process (e.g., via `Ctrl+c`) or run:

```bash
kraft rm modest_rancho
```
